...
class AdaptiveMaxPool2dPlugin : public IPluginV2 {
private:
...
public:
    ...
    AdaptiveMaxPool2dPlugin(std::vector<int64_t> output_size) : output_size(output_size) {}

    AdaptiveMaxPool2dPlugin(const char *data, size_t length) : AdaptiveMaxPool2dPlugin(std::string(data, length)) {}

    // create from serialized data
    AdaptiveMaxPool2dPlugin(const std::string &data) {
    ...
    int enqueue(int batchSize, const void* const* inputs, void** outputs, void* workspace, cudaStream_t stream) override {
        ...
        // enqueue work
        std::tuple<at::Tensor, at::Tensor> output_tmp = at::adaptive_max_pool2d(input, output_size);
        output.copy_(std::get<0>(output_tmp));
    ...
    IPluginV2* clone() const override {
        return new AdaptiveMaxPool2dPlugin(output_size);
    }
...
class AdaptiveMaxPool2dPluginCreator : public IPluginCreator {
public:
    AdaptiveMaxPool2dPluginCreator() {}
    ...
    IPluginV2 *deserializePlugin(const char *name, const void *data, size_t length) override {
        return new AdaptiveMaxPool2dPlugin((const char*) data, length);
    }
