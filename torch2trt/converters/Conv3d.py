...
def convert_Conv3d(ctx):
    ...
    kernel = trt.Weights(module.weight.detach().cpu().numpy())
    
    bias = trt.Weights(torch_dtype_to_trt(module.weight.dtype))
    if module.bias is not None:
        bias = trt.Weights(module.bias.detach().cpu().numpy())


    layer = ctx.network.add_convolution_nd(input_trt, module.out_channels,
        trt.Dims(kernel_size), kernel, bias)
