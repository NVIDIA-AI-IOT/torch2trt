...
class vanilla_cnn(torch.nn.Module):
    def __init__(self):
        ...
        self.layer1 = CR(3,3,3,padding=1,qconfig=qconfig)
        ...
        #self.layer1.weight = torch.nn.Parameter(torch.ones([3,3,3,3]))
...
input_t = torch.randint(16,[1,3,112,112],dtype=torch.float32)
...
trt_model = torch2trt(model,[input_t],int8_mode=True,QAT_mode=True,fp16_mode=True)

print("MODEL CONVERSION COMPLETE")
trt_out = trt_model(input_t)
trt_out = trt_out.detach().cpu().numpy()
print(np.shape(trt_out))
